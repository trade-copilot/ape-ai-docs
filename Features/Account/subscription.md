# Super Ape Subscription

Unlock the full AI trading copilot experience. One plan, all features.

**⏱️ Time to read:** 3 minutes
**📱🌐 Platform:** Both iOS & Web
**👤 Best for:** Active traders and investors

---

## 💎 Super Ape Plan

Ape AI offers **one premium plan** with everything you need to trade smarter.

**What's included:**
- ✅ **Unlimited AI Chat**
- ✅ **Unlimited searches**
- ✅ **Full ticker pages** with all data
- ✅ **Portfolio tracking** (unlimited accounts)
- ✅ **Ask Maverick** (portfolio AI)
- ✅ **Trade execution** from chat
- ✅ **All Quick Prompts**
- ✅ **Custom Prompts** (unlimited)
- ✅ **Advanced AI tools**
- ✅ **Priority support**
- ✅ **No ads**

**Pricing:**
- **Monthly:** $25/month
- **Annual:** $240/year (save $60 = 20% off)


---

## 🎯 Who is Super Ape For?

### Perfect for:

**✅ Active Traders**
- Multiple trades per week
- Need real-time portfolio tracking
- Want AI analysis of holdings

**✅ Speed-Focused Investors**
- Execute trades from chat instantly
- Trade multiple times daily
- Value convenience and speed

**✅ Serious Market Participants**
- Want professional-grade tools
- Need every edge
- Invest in their trading infrastructure

**✅ Research-Heavy Users**
- Spend hours on stock research
- Want AI to speed up analysis
- Value time savings

---

## 💡 Super Ape ROI

**If Super Ape helps you:**
- Avoid **ONE $25 mistake/month** = Pays for itself ✓
- Find **ONE good trade/month** = 10x+ ROI 📈
- Save **30 min/day** on research = Priceless ⏰

**$25/month for professional AI trading tools = Worth it**

**Quick math:**
- Monthly: $25 = less than 1 trade commission at most brokers
- Annual: $240 = $20/month = saves you from 1 bad trade/year

---

## 🔄 How to Subscribe

### 📱 iOS
1. Open Ape AI app
2. Tap **Profile** icon (top left)
3. Tap **Subscription** or **Upgrade to Super Ape**
4. Choose **Monthly** or **Annual**
5. Confirm with Face ID/Touch ID or Apple Pay

**Instant activation!**

---

### 🌐 Web
1. Go to app.askape.com
2. Click your **profile icon** (top right)
3. Select **Profile**
4. Click **Upgrade** or **Subscription**
5. Choose **Monthly** or **Annual**
6. Enter payment information
7. Confirm purchase

**Instant activation!**

---

## 📋 Subscription Management

### Change Your Plan

**Switch Monthly ↔ Annual:**
- iOS: Profile → Subscription → Manage → Change Plan
- Web: Profile → Subscription → Change Plan

**Annual saves you $60/year** (20% discount)

---

### Cancel Subscription

**iOS:**
1. Profile → Subscription
2. Tap **Manage Subscription**
3. Tap **Cancel Subscription**
4. Confirm cancellation

**Web:**
1. Profile → Subscription
2. Click **Cancel Subscription**
3. Confirm cancellation

**What happens when you cancel:**
- Keep access until current billing period ends
- No automatic renewal
- Can resubscribe anytime

**No hard feelings!** You can always come back. 🦍

---

## 💳 Payment & Billing

### Accepted Payment Methods
- 💳 Credit cards (Visa, Mastercard, Amex, Discover)
- 🍎 Apple Pay (iOS)
- 🌐 Debit cards
- 💰 Most major payment methods

### Billing Schedule
- **Monthly:** Charged every 30 days
- **Annual:** Charged once per year

### Payment Security
- 🔒 Secure payment processing via Stripe
- 🔐 We never store your card details
- ✅ Industry-standard encryption

---

## ❓ Frequently Asked Questions

**Q: Is there a free trial?**
A: Check your Profile → Subscription for current trial offers. Trial availability may vary.

**Q: What payment methods do you accept?**
A: Credit card, debit card, Apple Pay, and most major payment methods via Stripe.

**Q: Can I cancel anytime?**
A: Yes! No contracts, no commitment. Cancel anytime in your subscription settings.

**Q: What happens if I cancel?**
A: You keep full access until your current billing period ends. After that, your subscription will not renew.

**Q: Can I switch from monthly to annual (or vice versa)?**
A: Yes! You can switch plans anytime in your subscription settings. Billing adjusts automatically.

**Q: Do you offer refunds?**
A: Contact support@askape.com for refund requests. We review each case individually.

**Q: Can I get a receipt/invoice?**
A: Yes! Receipts are automatically emailed after each payment. You can also view payment history in your subscription settings.

**Q: What if my payment fails?**
A: You'll receive an email notification. Update your payment method in Settings to restore access.

**Q: Can I use Super Ape on multiple devices?**
A: Yes! One subscription works across iOS and Web. Just sign in with the same account.

**Q: Do you offer student discounts?**
A: Contact support@askape.com for special pricing inquiries.

**Q: Is Super Ape worth it?**
A: If you trade actively or value your time, absolutely. Avoiding one bad trade pays for months of Super Ape.

---

## 🆚 Super Ape vs Manual Research

### Without Super Ape:
- ⏰ Hours spent researching stocks manually
- 📚 Switching between 5+ websites for data
- 🤔 Guessing on trade setups
- 📊 No portfolio AI analysis
- 🐌 Slow order execution (switch to broker app)

### With Super Ape:
- ⚡ AI analyzes stocks in seconds
- 🎯 All data in one place
- 💡 AI suggests exact trade setups
- 🤖 Ask Maverick analyzes your portfolio
- 🚀 Execute trades instantly from chat

**Time saved per day: 30-60 minutes**
**Better decisions: Priceless**

---

## 🎁 What You Get Immediately

**After subscribing, you instantly unlock:**

1. **Unlimited AI Chat** - Ask anything, anytime
2. **Trade Execution** - Buy/sell right from chat
3. **Ask Maverick** - AI portfolio analysis
4. **All Quick Prompts** - Day trade setups, swing trades, etc.
5. **Custom Prompts** - Save your own prompts
6. **Advanced Tools** - Full ticker analysis, deep fundamentals
7. **Priority Support** - Faster help when you need it

**No waiting. All features, right now. 💎**

---

## 📈 Success Stories

**"Super Ape paid for itself in the first week. The AI found a trade setup I would've missed."** - Day Trader

**"Ask Maverick told me I was overexposed to tech. Rebalanced and avoided a 15% loss."** - Portfolio Manager

**"Executing trades from chat saves me so much time. I can act fast on opportunities."** - Active Investor

---

## What's Next?

**Ready to upgrade?**
- 📱 iOS: Profile → Subscription → Upgrade
- 🌐 Web: Profile → Subscription → Upgrade

**Want to learn more?**
- [Profile Settings →](profile) - Manage your account
- [Why Connect Brokerage →](../Brokerage/why-connect) - Unlock trade execution
- [Execute Trades from Chat →](../Chat/execute-trades) - See how it works

---

**Super Ape = Your Full AI Trading Copilot! 💎🦍**

*One plan. All features. Unlimited potential.*
