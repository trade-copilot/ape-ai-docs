# Bulls vs Bears

See what the market thinks. Sentiment analysis for every stock.

**⏱️ Time to read:** 3 minutes
**📱🌐 Platform:** Both iOS & Web
**👤 Best for:** Everyone

---

## 🐂🐻 What is Bulls vs Bears?

**Bulls vs Bears** shows you market sentiment - what traders, investors, analysts, and social media think about a stock.

**Bulls 🐂** = People who think stock will go UP
**Bears 🐻** = People who think stock will go DOWN

**Visual gauge:** 0% (Very Bearish) ←→ 100% (Very Bullish)

---

## 📊 What It Shows

### Sentiment Gauge

**Main visual:**
```
Bears 🐻 ←――――――●――――――→ Bulls 🐂
         0%     65%    100%
```

**Example:** 65% Bullish = More bulls than bears, positive sentiment

![Bulls vs Bears Gauge](https://raw.githubusercontent.com/trade-copilot/ape-ai-docs/main/Screenshots/Web/bulls-bears-gauge.jpg)

---

### Sentiment Sources

**1. Analyst Ratings** (30% weight)
- Buy/Sell/Hold ratings from Wall Street
- Price targets
- Upgrades/downgrades

**2. Social Media** (25% weight)
- Reddit r/WallStreetBets mentions
- Twitter ticker mentions
- StockTwits sentiment

**3. Options Flow** (20% weight)
- Put/Call ratio
- Unusual options activity
- Institutional options trades

**4. News Sentiment** (15% weight)
- Positive vs negative headlines
- AI analysis of article tone

**5. Institutional Activity** (10% weight)
- 13F filings (what hedge funds own)
- Insider buying/selling

---

## 🎯 Interpreting the Gauge

**90-100% Bullish** 🟢
- Extremely positive
- Everyone loves it
- **Risk:** Overcrowded trade, contrarian warning

**70-89% Bullish** 🟢
- Very positive
- Strong bull case
- **Good:** Momentum likely continues

**50-69% Bullish** 🟡
- Moderately positive
- Slight bull lean
- **Neutral:** Could go either way

**30-49% Bullish** 🟡
- Moderately bearish
- Slight bear lean
- **Caution:** Skepticism building

**10-29% Bullish** 🔴
- Very bearish
- Strong bear case
- **Opportunity?** Oversold contrarian play

**0-9% Bullish** 🔴
- Extremely bearish
- Everyone hates it
- **Risk:** Could be right or maximum pessimism

---

## 💡 How to Use Bulls vs Bears

### Confirming Your Thesis

**If you're bullish on a stock:**
- ✅ 70%+ bullish sentiment = Confirms your view
- ⚠️ 30% bullish sentiment = Maybe you're early or wrong?

### Contrarian Plays

**When sentiment is extreme:**
- 95% bullish? Maybe time to take profits (too crowded)
- 5% bullish? Maybe oversold opportunity (max pessimism)

**"Be fearful when others are greedy, greedy when others are fearful"**

### Momentum Trading

**For momentum plays:**
- Want sentiment increasing (40% → 60% → 80%)
- Sentiment shifting bullish = ride the wave
- Sentiment turning bearish = time to exit

---

## 📱 Where to Find It

**Ticker Page → Bulls vs Bears Section**
- Scroll down on ticker page
- Full sentiment breakdown
- Recent changes shown

![Bulls vs Bears Location](https://raw.githubusercontent.com/trade-copilot/ape-ai-docs/main/Screenshots/Web/bulls-bears-section.jpg)

---

## ❓ FAQ

**Q: How often does sentiment update?**
A: Real-time for social, daily for analyst ratings.

**Q: Can sentiment predict stock movement?**
A: No guarantees, but useful signal. Extreme sentiment often reverses.

**Q: What if sentiment conflicts with technicals/fundamentals?**
A: Use all data. Sentiment is one piece of puzzle.

---

## What's Next?

- [Ticker Overview →](overview) - All ticker features
- [Snapshot Grades →](snapshot-grades) - AI stock grades
- [Company Story →](company-story) - Business overview

---

**Bulls vs Bears = Know What the Market Thinks! 🐂🐻**
