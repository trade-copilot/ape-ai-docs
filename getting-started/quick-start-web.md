# Quick Start: Web

Get started with Ape AI on your desktop in 5 minutes.

---

## 🌐 Step 1: Access the Web App

1. Open your browser (Chrome, Safari, Firefox, Edge)
2. Go to **[app.Ape AI.com](https://app.Ape AI.com)**
3. You'll see the login/signup page

---

## 🔐 Step 2: Create Your Account

1. Click **Sign Up**
2. Enter your email and create a password
3. Verify your email (check your inbox)
4. Log back in

![Web Signup](../screenshots/web/signup.jpg)

**Or sign in with:**
- Google
- Apple
- Other SSO options

---

## 🗺️ Step 3: Dashboard Tour

Once logged in, you'll see the main dashboard:

![Web Dashboard](../screenshots/web/dashboard.jpg)

**Main sections:**

### Left Sidebar Navigation
- **Home** 🏠 - Market overview, news, trending
- **Chat** 💬 - AI trading assistant
- **Portfolio** 💼 - Your holdings and analysis

### Top Bar
- **Search** 🔍 - Find any stock
- **Profile** 👤 - Settings and account

---

## 🤖 Step 4: Ask Your First Question

Let's try the AI Chat:

1. Click **Chat** in the left sidebar (or press `/` then type)
2. You'll see the chat interface with your chosen persona
3. Click the **Prompt icon** (or press `Ctrl+P`) to see Quick Prompts
4. Select **"Day Trade Options Setup"**
5. Wait for the AI to respond
6. Read the analysis!

![Web Chat](../screenshots/web/chat.jpg)

**Or type your own question:**
- Click in the text box at bottom
- Type: *"What are the best tech stocks under $100?"*
- Press Enter or click Send
- Get personalized AI analysis!

**Keyboard Shortcuts:**
- `Ctrl+P` - Open prompts
- `Ctrl+H` - Chat history
- `/` - Focus search
- `Enter` - Send message

---

## 📈 Step 5: Research a Stock

Let's look up a stock:

1. Click the **Search bar** at the top (or press `/`)
2. Type a ticker symbol (e.g., **"NVDA"**)
3. Click the result or press Enter

![Web Stock Search](../screenshots/web/search.jpg)

**You'll see the ticker page:**

![Web Ticker Page](../screenshots/web/ticker-page.jpg)

**Key sections:**
- **Price Chart** - Interactive, multiple timeframes
- **AI Snapshot Grades** - 4 letter grades (F, T, S, R) ⭐
- **Market Stats** - Cap, P/E, Volume, 52W range
- **Bulls vs Bears** - Sentiment analysis
- **Company Story** - 4 expandable cards
- **Performance** - vs S&P 500
- **Related Companies** - Similar stocks

**Click any Snapshot Grade** to expand and see full AI reasoning.

---

## 📰 Step 6: Explore the Feed

Check what's hot:

1. Click **Home** in the left sidebar
2. Scroll through the page

![Web Home/Feed](../screenshots/web/home-feed.jpg)

**You'll see:**
- **Market Indices** (SPY, QQQ, IWM, DIA)
- **Banana Bites** 🍌 - Hot stocks quick access
- **Latest News** - Personalized feed
- **Today's Recap** - Daily market summary
- **Trending Stocks** - Most Active, Top Gainers, etc.
- **Themes** - Suggested stock lists

---

## 💼 Step 7: Set Up Portfolio (Optional)

Track your investments:

1. Click **Portfolio** in the left sidebar
2. Click **Connect** a brokerage

![Web Portfolio](../screenshots/web/portfolio.jpg)

**Two options:**

### Option A: Ape Paper Trading (FREE) 📄
Perfect for beginners or practicing!
1. You'll see "Ape Paper Trading" listed first
2. Click **Claim Account**
3. Get virtual money to practice
4. Start trading risk-free!

### Option B: Connect Real Brokerage 🏦
1. Select your brokerage from supported list
2. Click Connect and authorize
3. Your positions sync automatically
4. Track everything in one place

---

## 🎨 Step 8: Create Your First Theme

Themes are like custom stock screeners:

1. Go to **Home** page
2. Scroll to **Themes** section
3. Click **"Create a theme"**
4. Type your criteria in natural language:
   - *"Tech stocks under $50 with positive earnings"*
   - *"Dividend stocks with 5%+ yield"*
   - *"EV companies with strong momentum"*
5. Name your theme
6. Save and track it!

![Create Theme](../screenshots/web/create-theme.jpg)

---

## ⚡ Quick Tips for Web

### Keyboard Shortcuts
- **`/`** - Focus search bar
- **`Ctrl+P`** - Open prompts
- **`Ctrl+H`** - Chat history
- **`Ctrl+K`** - Quick command menu (if available)
- **`Esc`** - Close modals

### Multi-Window Workflow
Open multiple tabs:
- Tab 1: Chat for AI questions
- Tab 2: Ticker page for research
- Tab 3: Portfolio for monitoring

### Bookmarks
Bookmark frequently used:
- `app.Ape AI.com` - Home
- `app.Ape AI.com/chat` - Chat
- `app.Ape AI.com/portfolio` - Portfolio
- Specific tickers: `app.Ape AI.com/tickers/TSLA`

---

## 🎯 What's Next?

Now that you're set up:

**Choose Your Persona:**
- [What Are Personas?](../features/personas/what-are-personas.md)
- [Meet the Personas](../features/personas/meet-the-personas.md)

Note: Persona selection is currently iOS-only but coming to web soon!

**Learn the Features:**
- [AI Chat Basics](../features/chat/basics.md)
- [Understanding Snapshot Grades](../features/ticker/snapshot-grades.md)
- [What Are Themes?](../features/themes/overview.md)

**Try a Workflow:**
- [My First Stock Research](../workflows/beginner/first-research.md)
- [Premarket Day Trading Routine](../workflows/advanced/premarket-routine.md)

---

## 📱 Want Mobile Too?

The iOS app syncs with web! Use both:
- **Web** for deep research and analysis
- **iOS** for quick checks on-the-go

→ [Download iOS App](quick-start-ios.md)

---

## ❓ Need Help?

- [Web-Specific Features](../platforms/web-guide.md)
- [Keyboard Shortcuts Guide](../platforms/web-guide.md#keyboard-shortcuts)
- [FAQ](../support/faq.md)
- [Troubleshooting](../support/troubleshooting.md)

---

**Welcome to the ape army! 🦍🚀**

Ready to find some alpha! Let's go →

---

**Platform Comparison:**
Not sure which to use? → [iOS vs Web Comparison](../platforms/comparison.md)
